...
export class NgxWidgetResizerDirective {
  ...
  onMove(event: MouseEvent) {
    if (this.enableDrag === this.widgetCmp.getConfig().id) {
      ...
      if (this.moveUpAllowed) {
        delta.top =
          Math.min(
            Math.max(dragPositionY - this.draggerOffset.top, 0),
            gridDims.height
          ) - startRender.top;
        ...
        delta.bottom =
          startRender.bottom -
          Math.min(
            Math.max(dragPositionY - this.draggerOffset.bottom, 0),
            gridDims.height
          );
      ...
      if (this.moveLeftAllowed) {
        delta.left =
          Math.min(
            Math.max(dragPositionX - this.draggerOffset.left, 0),
            gridDims.width
          ) - startRender.left;
        ...
        delta.right =
          startRender.right -
          Math.min(
            Math.max(dragPositionX - this.draggerOffset.right, 0),
            gridDims.width
          );
  ...
  findCollision(
    ...
    for (let i = start; i <= end; i++) {
      const checker = reverse
        ? this.gridCmp.isPointObstructed(i, val)
        : this.gridCmp.isPointObstructed(val, i);
      if (checker) {
        foundCollision = true;
        break;
      }
  ...
  determineFinalPos(): any {
    ...
    const startRender = this.startRender;
    const delta = this.delta;
    const requestedStartPoint = this.gridCmp.rasterizeCoords(
      startRender.left + delta.left + 1,
      startRender.top + delta.top + 1
    );
    const requestedEndPoint = this.gridCmp.rasterizeCoords(
      startRender.right - delta.right - 1,
      startRender.bottom - delta.bottom - 1
    );
    ...
    if (this.moveUpAllowed && requestedPos.top < this.startPosition.top) {
      ...
      this.moveDownAllowed &&
      requestedPos.bottom > this.startPosition.bottom
    ) {
    ...
    if (this.moveLeftAllowed && requestedPos.left < this.startPosition.left) {
      ...
      while (finalPos.left > requestedPos.left) {
        ...
        foundCollision = this.findCollision(
          finalPos.top,
          finalPos.bottom,
          finalPos.left - 1,
          true
      ...
      this.moveRightAllowed &&
      requestedPos.right > this.startPosition.right
    ) {
