...
export class NgxWidgetMoverDirective {
  ...
  onDown(event: MouseEvent) {
    if (event.button === 0) {
      ...
      this.desiredPosition = {
        top: this.startRender.top,
        left: this.startRender.left,
      };
      ...
      this.cellHeight =
        (this.gridCmp.grid.cellSize.height / 100) * this.gridPositions.height;
      this.cellWidth =
        (this.gridCmp.grid.cellSize.width / 100) * this.gridPositions.width;
      this.enableDrag = this.widgetCmp.getConfig().id;

      this.renderer.setStyle(
        this.widgetCmp.getEl().nativeElement,
        "z-index",
        this.ngxWidgetMover ? 0 : 100
      );
  ...
  onMove(event: MouseEvent) {
    if (this.enableDrag === this.widgetCmp.getConfig().id) {
      ...
      const currentFinalPos: Rectangle = this.determineFinalPos(
        this.startPosition,
        desiredPosition,
        this.startRender,
        this.cellHeight,
      ...
      this.renderer.setStyle(
        this.widgetCmp.getEl().nativeElement,
        "top",
        desiredPosition.top + "px"
      );
      this.renderer.setStyle(
        this.widgetCmp.getEl().nativeElement,
        "left",
        desiredPosition.left + "px"
      );
    }
  }
  ...
  onUp(event: MouseEvent) {
    if (this.enableDrag === this.widgetCmp.getConfig().id) {
      ...
      const anchorTop = this.getAnchor(
        Math.max(dragPositionY, 0),
        this.cellHeight,
        1
      );
      const anchorLeft = this.getAnchor(
        Math.max(dragPositionX, 0),
        this.cellWidth,
        1
      );
      const dropPosition: any = this.gridCmp.rasterizeCoords(
        anchorLeft,
        anchorTop
      );
      ...
      if (obstructingWidgetId && this.ngxWidgetMover) {
        const obstructingWidgetCmp =
          this.gridCmp.getWidgetById(obstructingWidgetId);
        const obstructingWidgetPosition =
          this.gridCmp.getWidgetPositionByWidgetId(obstructingWidgetId);
        ...
        finalPos = this.determineFinalPos(
          this.startPosition,
          desiredPosition,
          this.startRender,
          this.cellHeight,
  ...
  getAnchor(val: number, cellWOrH: number, marginFactor = 2): number {
    return val % cellWOrH > cellWOrH / marginFactor
      ? val + Math.floor(cellWOrH)
      : val;
  }
  ...
  determineFinalPos(
    ...
    if (
      startRender.top === desiredPos.top &&
      startRender.left === desiredPos.left
    ) {
      return startPos;
    }
    ...
    const desiredFinalPosition: any = this.gridCmp.rasterizeCoords(
      anchorLeft,
      anchorTop
    );
    ...
    while (path && path.hasNext()) {
      ...
      if (
        this.ngxWidgetMover &&
        this.gridCmp.isAreaObstructed(targetArea, options)
      ) {
      ...
      if (!this.gridCmp.isAreaObstructed(targetArea, options)) {
        ...
        if (desiredFinalPosition.top < targetArea.top) {
          while (desiredFinalPosition.top <= targetArea.top - 1) {
            const checkRect = new Rectangle({
              top: targetArea.top - 1,
              left: targetArea.left,
              height,
              width,
            });
            const isRectVacant = !this.gridCmp.isAreaObstructed(
              checkRect,
              options
            );
            if (isRectVacant) {
