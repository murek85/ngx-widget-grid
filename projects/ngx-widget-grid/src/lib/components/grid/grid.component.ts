...
export class NgxWidgetGridComponent implements AfterViewInit {
  ...
  @Output() public widgetPositionChange: EventEmitter<WidgetPositionChange> =
    new EventEmitter<WidgetPositionChange>();
  @Output() public gridFull: EventEmitter<boolean> =
  ...
  rasterizeCoords(x: number, y: number): Cell {
    return this.gridRenderer.rasterizeCoords(
      x,
      y,
  ...
  updateWidget(widget: NgxWidgetComponent, swappingWidgets: boolean) {
    ...
    this.gridRenderer.setWidgetPosition(
      config.id,
      newPosition,
      swappingWidgets
    );
  ...
  isAreaObstructed(
    ...
    return this.gridRenderer
      ? this.gridRenderer.isAreaObstructed(area, options)
      : true;
  }

  areaObstructor(area: Rectangle) {
    return this.gridRenderer.obstructions[
      (area.top - 1) * this.grid.columns + (area.left - 1)
    ];
  }
  ...
  getWidgetById(widgetId: string): NgxWidgetComponent {
    return (
      this.widgetComponents.find((widgetCmp) => {
