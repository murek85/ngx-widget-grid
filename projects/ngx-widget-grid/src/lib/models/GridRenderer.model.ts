...
export class GridRenderer {
  ...
  getWidgetIdAt(i: number, j: number) {
    for (const widgetId in this.positions) {
      if (this.positions.hasOwnProperty(widgetId)) {
        ...
        if (
          position.top <= i &&
          i <= position.top + position.height - 1 &&
          position.left <= j &&
          j <= position.left + position.width - 1
        ) {
          return widgetId;
        }
      }
  ...
  setWidgetPosition(
    ...
    newPosition = new Rectangle({
      ...
      left: Utils.isNumber(newPosition.left)
        ? newPosition.left
        : currPosition.left,
      width: Utils.isNumber(newPosition.width)
        ? newPosition.width
        : currPosition.width,
      height: Utils.isNumber(newPosition.height)
        ? newPosition.height
        : currPosition.height,
    });
  ...
  isAreaObstructed(
    ...
    if (
      !Utils.isNumber(top) ||
      !Utils.isNumber(left) ||
      !Utils.isNumber(bottom) ||
      !Utils.isNumber(right)
    ) {
      return false;
    }
  ...
  findLargestEmptyArea(): Rectangle {
    ...
    for (let i = 1; i <= this.grid.rows; i++) {
      for (let j = 1; j <= this.grid.columns; j++) {
        ...
        const currAreaLimit =
          (this.grid.rows - i + 1) * (this.grid.columns - j + 1);
  ...
  public _findLargestEmptyAreaFrom(start: Cell) {
    if (
      !Utils.isDefined(this.grid) ||
      !Utils.isNumber(this.grid.columns) ||
      !Utils.isNumber(this.grid.rows)
    ) {
      return null;
    }
  ...
  render(grid: Grid, emitWidgetPositionUpdated?: Function) {
    ...
    const widgets: WidgetConfig[] =
      this.grid && this.grid.widgets ? this.grid.widgets : [];
