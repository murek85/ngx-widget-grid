...
export class PathIterator {
  ...
  constructor(start: Rectangle, end: Rectangle) {
    ...
    this._steps = Math.max(
      Math.abs(this._heightDelta),
      Math.abs(this._widthDelta)
    );
    this._currStep = 0;
    this._currPos = null;
  ...
  public next(): Cell {
    ...
    if (this._currStep < this._steps) {
      this._currStep++;
      const currTopDelta = Math.round(
        (this._currStep / this._steps) * this._heightDelta
      );
      const currLeftDelta = Math.round(
        (this._currStep / this._steps) * this._widthDelta
      );
      this._nextPos = new Cell(
        this._start.top + currTopDelta,
        this._start.left + currLeftDelta
      );
    } else {
      this._nextPos = null;
    }
